...
const ENDPOINT = "/api/encounter/dm/campaign-dm-notes";

export function useCampaignDmNotes(campaignId: string | null | undefined) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error || `failed_to_load_dm_notes_http_${res.status}`);
      ...
      setState((s) => ({
        ...s,
        isLoading: false,
        error: e?.message || "failed_to_load_dm_notes",
      }));
    }
  }, [canLoad, campaignId]);
  ...
  const flushSave = useCallback(
    async (nextValue: string) => {
      ...
      try {
        ...
        const json = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(json?.error || `failed_to_save_dm_notes_http_${res.status}`);
        ...
        setState((s) => ({
          ...s,
          isSaving: false,
          error: e?.message || "failed_to_save_dm_notes",
        }));
      }
    },
    [canLoad, campaignId]
